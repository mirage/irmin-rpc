...
struct
  ...
  let rec encode_tree tr key (tree : Store.tree) : unit Lwt.t =
    ...
    | `Contents (contents, _) ->
        let _ =
          Tree.contents_set tr (Fmt.to_to_string Store.Contents.pp contents)
        in
        Lwt.return_unit
    | `Tree l ->
        Lwt_list.map_p
    ...
    | Node l ->
        ...
        `Tree
          (List.map
             (fun node ->
               let step =
                 Node.step_get node |> Store.Key.step_of_string |> unwrap
               in
  ...
  let encode_commit commit cm =
    ...
    Commit.hash_set commit
      (Fmt.to_to_string Store.Commit.Hash.pp (Store.Commit.hash cm));
  ...
  let local ctx =
    ...
    @@ object
           ...
           Service.return_lwt (fun () ->
               ...
               | Some value ->
                   Results.result_set results
                     (Fmt.to_to_string Store.Contents.pp value);
                   Lwt.return_ok resp
               | None ->
                   Lwt.return_ok resp )
         ...
         method merge_impl req release_params =
           ...
           let from_ =
             Params.branch_from_get req |> Store.Branch.of_string |> unwrap
           in
           let into_ =
             Params.branch_into_get req |> Store.Branch.of_string |> unwrap
           in
           let message =
         ...
         method commit_info_impl req release_params =
           ...
           Service.return_lwt (fun () ->
               let resp, results =
                 Service.Response.create Results.init_pointer
               in
               let hash = Store.Commit.Hash.of_string hash |> unwrap in
           ...
           Service.return_lwt (fun () ->
               ...
               | Some commit ->
                   Results.result_set results
                     ...
                     |> Fmt.to_to_string Store.Commit.Hash.pp );
                   Lwt.return_ok resp
               | None ->
                   Lwt.return_ok resp )
         ...
         method revert_impl req release_params =
           ...
           in
           let commit =
             Params.hash_get req |> Store.Commit.Hash.of_string |> unwrap
           in
           release_params ();
           Service.return_lwt (fun () ->
               let resp, results =
         ...
         method branches_impl _req release_params =
           ...
           Service.return_lwt (fun () ->
               ...
               let l =
                 List.map
                   (fun x -> Fmt.to_to_string Store.Branch.pp x)
                   branches
               in
  ...
  module Client = struct
    ...
    let find t ?branch key =
      ...
      if Results.has_result res then
        Some (Store.Contents.of_string (Results.result_get res) |> unwrap)
      else None
